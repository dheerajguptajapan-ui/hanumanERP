...
export function NewVendor({ onClose, editingId }: NewVendorProps) {
  ...
  useEffect(() => {
    const loadVendor = async () => {
      if (editingId) {
        const vendor = await db.partners.get(editingId);
        if (vendor) {
          form.setValues({
            salutation: 'Mr.', 
            ...
            registrationType: (vendor as any).registrationType || 'Regular',
            billingAttention: vendor.billingAttention || '',
            ...
            billingFax: vendor.billingFax || '',
            shippingAttention: vendor.shippingAttention || '',
            shippingCountry: vendor.shippingCountry || 'India',
            ...
            shippingFax: vendor.shippingFax || '',
            contactPersons: vendor.contactPersons || [],
            attachments: (vendor as any).attachments || [],
          });
        }
      }
    };
  ...
  const handleSave = async (values: typeof form.values) => {
    try {
      const vendorData = {
        ...
        email: values.email,
        customerType: 'Business' as any, // Vendors are usually business
      ...
      if (editingId) {
        await db.partners.update(editingId, vendorData);
        notifications.show({ title: 'Success', message: 'Vendor updated successfully', color: 'green' });
      } else {
        await db.partners.add(vendorData);
        notifications.show({ title: 'Success', message: 'Vendor created successfully', color: 'green' });
      }
      onClose();
    } catch (error) {
      notifications.show({ title: 'Error', message: 'Failed to save vendor', color: 'red' });
    }
  };
    ...
    <Box bg="white" h="100%" style={{ display: 'flex', flexDirection: 'column' }}>
      ...
      <Box p="md" style={{ borderBottom: '1px solid #e2e8f0', backgroundColor: 'white', position: 'sticky', top: 0, zIndex: 10 }}>
        <Group justify="space-between">
          <Title order={3} fw={500}>{editingId ? 'Edit Vendor' : 'New Vendor'}</Title>
            ...
            <Stack gap="xl">
              <Box>
                ...
                <Grid align="center" mb="md">
                  <Grid.Col span={2}>
                    <Group gap="xs">
                      <Text size="sm">Vendor Language</Text>
                      <HelpCircle size={14} color="#adb5bd" />
                    </Group>
                  </Grid.Col>
                  <Grid.Col span={10}>
                  ...
                  <Tabs.Panel value="other_details" pt="xl">
                    <Stack gap="lg">
                      ...
                      <Grid align="center">
                        ...
                        <Grid.Col span={10}>
                          <Checkbox label="Allow portal access for this vendor" {...form.getInputProps('enablePortal', { type: 'checkbox' })} />
                        </Grid.Col>
                      </Grid>
                    </Stack>
                  </Tabs.Panel>
